...
export default function SiteHeader({ onSearchClick }) {
  ...
  const handleDateToggleClick = (e) => {
    e.stopPropagation();
    if (isMobileViewport()) {
      onSearchClick?.();      // ⬅ на мобилке открываем твой флоу
      return;
    }
    setIsDateOpen((prev) => !prev); // на десктопе — как было
  };
          ...
          <form className="listing-search mx-auto d-flex align-items-center">
            <div className="search-line d-flex align-items-center">
              <div className="search-item search-item-input">
                <input
                  ...
                  onFocus={(e) => {
                    if (isMobileViewport()) {
                      e.target.blur();
                      onSearchClick?.();   // ⬅ на мобилке фокус в поле открывает флоу, а не клавиатуру
                      return;
                    }
                    e.target.placeholder = '';
                  }}
                  onBlur={(e) => {
                    if (!e.target.value) e.target.placeholder = t('header.regionLabel');
                  }}
                />
              </div>
              ...
              <div
                ...
                onClick={() => { if (isMobileViewport()) onSearchClick?.(); }}  // ⬅ добавили
              >
