...
export default function MobileSearchFlow({ isOpen, onClose, onApply, initialState }) {
  ...
  const setChildren = (next) => {
    ...
    setBooking((prev) => {
      ...
      while (childAges.length < clamped) childAges.push(0);
  ...
  const submitSearch = () => {
    ...
    const appliedSearchMode = hasValidDates ? "По датам" : "По месяцам"; // 8.3

    const finalState = {
      ...booking,
      // 4.5.8 — при поиске по месяцам ранее выбранные точные даты не сохраняются
      checkIn: appliedSearchMode === "По датам" ? booking.checkIn : null,
      checkOut: appliedSearchMode === "По датам" ? booking.checkOut : null,
    };

    onApply?.({ ...finalState, appliedSearchMode }); // 8.2 / 8.3
  };

  // ---------- сводки для шапки «Кто» (7.3 / 7.4) ----------
  const whoSummaryLine = (() => {
    if (booking.mode === "months") {
      if (booking.arrivalMonths.length > 0) {
        return `${booking.stayMonths} ${monthWord(booking.stayMonths)} · ${booking.arrivalMonths
          .map(formatMonthChip)
          .join(", ")}`;
      }
      return `${booking.stayMonths} ${monthWord(booking.stayMonths)}`;
    }

    if (booking.checkIn && booking.checkOut) {
      return formatDatesSummary(booking.checkIn, booking.checkOut);
    }
          ...
          <>
            ...
            <div className="sf-body">
              <div className="sf-who-context">
                ...
                <div className="sf-who-summary">{whoSummaryLine}</div>
              </div>
              ...
              {booking.childAges.map((age, index) => (
                <div className="sf-who-row" key={index}>
                  <div className="sf-who-row-title">Возраст ребёнка {index + 1}</div>
                  <select
                    className="sf-age-select"
                    value={age}
                    onChange={(e) => setChildAge(index, Number(e.target.value))}
                  >
                    {Array.from({ length: 18 }, (_, i) => i).map((v) => (
