...
const getEffectiveAvailableMonthStart = (availableFromIso) => {
  const today = new Date();
  const todayMonthStart = new Date(today.getFullYear(), today.getMonth(), 1);
  const availableMonthStart = getMonthStart(availableFromIso);

  return availableMonthStart.getTime() > todayMonthStart.getTime()
    ? availableMonthStart
    : todayMonthStart;
};
...
const [calendarMonthDate, setCalendarMonthDate] = useState(
  getEffectiveAvailableMonthStart(availableFromIso)
);
...
const availableFromMonthStart = getEffectiveAvailableMonthStart(availableFromIso);
...
const resetDraftDates = () => {
  ...
  //  «Сброс дат» всегда возвращает к исходному состоянию —
  // без дат, минимальный срок объекта, календарь на первом доступном
  // будущем месяце (позже из available_from и текущего месяца).
  setCalendarMonthDate(getEffectiveAvailableMonthStart(availableFromIso));
...
const openCalendarModal = () => {
  ...
  setCalendarMonthDate(
    bookingState.checkIn
      ? getMonthStart(bookingState.checkIn)
      : getEffectiveAvailableMonthStart(availableFromIso)
  );
  setDraftDateEditMode(null);
              ...
              {mobileStayStep === "calendar" && (
                <>
                    ...
                    <div className="object-mobile-calendar">
                      <div className="object-mobile-calendar-summary">
                        ...
                        <div className="object-mobile-calendar-summary-sub">
                          {draftBookingState.checkIn && draftBookingState.checkOut ? (
                            <span className="object-summary-value">
                              {formatDateRange(draftBookingState.checkIn, draftBookingState.checkOut)}
                            </span>
                          ) : draftBookingState.checkIn ? (
                            <>
                              <span className="object-summary-label">Заезд:</span>{" "}
                              <span className="object-summary-value">{formatDisplayDate(draftBookingState.checkIn)}</span>
                              {" · "}
                              <span className="object-summary-label">Выезд не раньше</span>{" "}
                              <span className="object-summary-value">{formatDisplayDate(addDays(draftBookingState.checkIn, minStayDays))}</span>
                            </>
                          ) : (
                            "Укажите даты"
                          )}
                              ...
                              if (!day) {
                                return (
                                  <span
                                    ...
                                    className="object-calendar-day is-invisible"
                                  />
                                );
                              }
                  ...
                  <div className="object-mobile-fixed-cta d-lg-none">
                    <button
                      type="button"
                      className="object-mobile-guests-row mb-2"
                    ...
                    <div className="object-mobile-fixed-cta-row">
                      <button
                        type="button"
                        className="object-mobile-reset-dates-button"
